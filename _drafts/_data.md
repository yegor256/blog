...
interfaces making raw data visible, letting classes deal with them, parse
and turn into smaller data elements. You may be surprised, but I'm
suggesting the second option as more elegant. It seems to me that we don't
loose object orientation, but gain a lot of flexibility, reusability, testability,
and so on.
...
Take a look at this (let's call it _fat_ and I will explain later why):
...
* **Extendability**.
  ...
  which are managed and parsed later, in decorators, adapters, and other
...
* **Reusability**.
  ...
  parsing of author's information is required, while class `PgAuthor` is
...
* **Testability**.
  ...
  of a single interface is a much simpler task that mocking of the entire
...
and for 2) to-PostgreSQL data manipulations. Of course, manipulations may
...
gets born. We want everything data-related to happen where the data stays:
inside `SqlArticle`.

This is a valid concern, but letting PostgreSQL-related information (like connection
settings) move from `PgArticle` to `PgHead` and then to `PgAuthor` is even
larger violation of the data encapsulation principle.

In real-life situations, of course, it's not possible to imagine pure
one-interface skinny designs. They all will be fat to some extent. My suggestion though
is to try to make designs less fat, letting interface users
dress them up however they feel like. This suggestion is very close to
