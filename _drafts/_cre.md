...
a chapter about Risk Management of her great book
...
from these books and my multi-years [experience](http://www.technoparkcorp.com/process/risk/risk/)
of identifying, analysing and dealing with risks is that the best
format of specifying them consists of three parts: cause, risk, and effect.

<!--more-->

{% jb_picture_body %}

Let's start with a simple example. I've developed [Sibit](https://github.com/yegor256/sibit),
...
stop using it. They will also think of me as a creator of some garbage that
...
make sure my CI runs the build every, say, day. Once the build goes
...
say week, to make sure it's still in a good shape and works against
the API.

Now, let's turn this story into a formal risk management. I took the titles
of subsections below from the
...
{% highlight text %}
Cause #1: Sibit works through Blockchain API
...
"Since Sibit works through Blockchain API the API may be changed without notice
and users will be disappointed." But it's better to clearly
define cause/risk/effect components, because, guess what ... we may
have different combinations of risks, effects and causes, in any possible
way. For example, how about identifying an additional risk for an existing cause:
...
but will entirely disappear from the market. Is it possible? Pretty much.
...
will have a significantly different design. In other words, the effect of
the risk #2 will sound like:
...
about it at the right moment of time, we may _exploit_ that opportunity
and create a similar API for other users, right? Thus, the risk #2 has
an additional effect, which sounds like:
...
such a multi-level diagram and avoid duplication of texts
...
There are some rules I learned for myself, about those three parts:
  ...
  * **Does**.
    ...
    [present tense](https://en.wikipedia.org/wiki/Present_tense),
    since it states the fact that exists right now, e.g.
    "we use Hibernate,"
  ...
  * **May**.
    The Risk should have the word "may" inside,
  ...
  * **Will**.
    ...
    [future tense](https://en.wikipedia.org/wiki/Future_tense), explicitly
    stating the fact we will experience in the future if the risk occurs, e.g.
    "we will bankrupt,"
    "we will have to rewrite the entire module,"
    "we will spent another $10,000 for hardware,"
    and so on.

Needless to say that the shorter the texts, the better. A properly
defined cause, risk, and effect must include up to 20 words each. Longer
texts mean only one thing: the author is not clear about what's going on
and need to spend more time to investigate the situation.

## Qualitative Analysis

Not all risks and effects are equally possible, as you can see. It is very
unlikely that the entire Blockchain API will die, but it's very likely
that it may change its HTTP protocol. It will be only silly to pay
...
where one means that the risk most likely will never occur and nine
means that the risk will happen undoubtfully:
...
I put 7 and 2 to the risks above. It was my _expert judgement_. No math here.
I just took a look at the risks and made my personal decision,
as an owner/manager of the project.

Then, we assign an _impact_ to each effect, again within the `[1..9]` interval.
Here, one means that the consequences we are expecting won't hurt anyone
and won't really help anyone, while nine means that the effect is crucial
...
Again, I put the numbers according to my expert judgement. Changing the gem
...
You can now see who is who in our list. Even though the consequences of the
...
Quantitative risk analysis is not required for all projects or for all
risks. It may be skipped in favor of moving on to risk response planning."
...
two options for me in each _positive_ line of my _Risk List_:
  ...
  * **Avoid**.
    ...
    longer and don't die? Well, I can tweet about it, promote it, or maybe
    even donate some money to them. But I doubt any of that will really help.
...
or transferring_ (find a scape goat who to blame when things go south), but they
are less practical.
...
There are two options for a _good_ risk (E1/R2/E3) too:
  ...
  * **Exploit**.
    ...
    like a joke in this case, but very often we may somehow push things
  ...
  * **Enhance**.
    ...
    like "Hey, those guys are dead, switch here now!" I'm kidding, but you
    got the idea.
...
may also be a solution in some cases (not in this one).
...
Make sense?
I hope I do. I definitely recommend you reading
[_Risk Management Tricks of the Trade for Project Managers_](https://amzn.to/2V5b0wp).
It explains it all in many more details and it's fun to read.
...
(it's in [Zerocracy](https://www.zerocracy.com) product family, that's why
...
The interface is pretty intuitive, you should have no issues. If any
...
plans into _tasks_, when time comes. The tasks are explicit instructions
for you, a project manager. Then, you do them yourself or delegate to your
project members.
...
productive and results focused. First, I identify what are the most important
situations in my project (which could be people, resources, bank accounts,
...
Thanks to that, I don't miss the overall scope---it is defined by the
