...
my code works together with "real" DynamoDB server/tables. Let me
show you how it was solved. The code is open source, you can check it
in [yegor256/sixnines](https://github.com/yegor256/sixnines) GitHub repo.

<!--more-->

{% jb_picture_body %}

## How to boostrap the DynamoDB Local
...
I'm suggesting to use [`jcabi-dynamodb-maven-plugin`](http://dynamodb.jcabi.com/),
a [Maven](https://maven.apache.org/) plugin that I've
...
repository and start/stop Maven from
[`Rakefile`](https://github.com/yegor256/sixnines/blob/0.17/Rakefile),
just like I'm doing it [here](https://github.com/yegor256/sixnines/blob/0.17/Rakefile#L50-L67):
...
Then, I'm checking the status of `sn-endpoints`, one of the tables in the
...
Most probably the table won't be ready immediately, since it takes time to
bootstrap Maven, start the server, and create tables there. That's why the
exception will be thrown, I will catch it, will wait for 5 seconds and try again.
I will keep trying many times, until the server is ready. Eventually it will.
It takes about 12-15 seconds on my MacBook, which means 2-3 attempts/exceptions.

## How to connect to the DynamoDB Local

My classes need to know how to connect to the server during integration tests.
In production they need to connect to the AWS, in testing they have to
know about the DynamoDB Local I just started. This is what I have in my class
...
The TCP port of the DynamoDB Local will be
[there](https://github.com/yegor256/sixnines/blob/0.17/dynamodb-local/config/dynamo.yml),
as well as DynamoDB authentication key and secret.
...
Now you can run the entire test from command line. You can see how
...
In a nutshell:
  ...
  * You call `rake` in command line and Rake starts;
  * Rake attempts to run [`default`](https://github.com/yegor256/sixnines/blob/0.17/Rakefile#L32) task,
  ...
  * Maven [saves](https://github.com/yegor256/sixnines/blob/0.17/dynamodb-local/pom.xml#L79-L100)
    `${dynamo.port}` and key/secret pair into;
  ...
  * Rake [waits](https://github.com/yegor256/sixnines/blob/0.17/Rakefile#L57-L65)
    for the DynamoDB Local availability on the reserved port;
